...
AttentionCommand(){
   ...
   return flag;
    ...
    public void setCmdconfig() {
        new AttentionConfigView().getConfig(this);
    }
    ...
    public void run(Trigger current) {
        ...
        while(!flag){
            try {
                attention=EegMachine.attention;
                //count clock if meditation is detected and clock not running
                if(operation.equals("greater than (>)") && attention > Integer.parseInt(threshold)){detectflag=true;}
                else if(operation.equals("lesser than (<)") && attention < Integer.parseInt(threshold)){detectflag=true;}
                else if(operation.equals("equals (=)") && attention == Integer.parseInt(threshold)){detectflag=true;} 
