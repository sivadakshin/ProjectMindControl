 ...
 * @author siva
 */
public class Low_betaCommand implements Command {
...
Low_betaCommand(){
   ...
   return flag;
    ...
    public void setCmdconfig() {
        new Low_betaConfigView().getConfig(this);
    }
    ...
    public void run(Trigger current) {
        ...
        while(!flag){
            try {
                low_beta=EegMachine.low_beta;
                //count clock if low_beta is detected and clock not running
                if(operation.equals("greater than (>)") && low_beta > Integer.parseInt(threshold)){detectflag=true;}
                else if(operation.equals("lesser than (<)") && low_beta < Integer.parseInt(threshold)){detectflag=true;}
                else if(operation.equals("equals (=)") && low_beta == Integer.parseInt(threshold)){detectflag=true;} 
